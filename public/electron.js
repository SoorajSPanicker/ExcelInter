...
function createDatabase() {
    ...
    db = new sqlite3.Database(dbPath, (err) => {
        if (err) {
            ...
            db.run('CREATE TABLE IF NOT EXISTS ShipSchedule (excelId TEXT, projId TEXT, place TEXT, projNo TEXT, startDate TEXT, endDate TEXT, workDays Text, PRIMARY KEY(projId))')
...
app.whenReady().then(() => {
    ...
    ipcMain.on('excel-data-save', (event, data) => {
        ...
        const projectDb = new sqlite3.Database(databasePath, (err) => {
            ...
            projectDb.run(
                'INSERT INTO ShipSchedule (excelId ,projId ,place ,projNo ,startDate ,endDate ,workDays ) VALUES (?, ?, ?, ?, ?, ?, ?)',
                [data.excelId, data.projId, data.place, data.projNo, data.startDate, data.endDate, data.workDays],
                function (err) {
                    if (err) {
